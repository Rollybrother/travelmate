      ...
      <div class="row mx-5 mt-3"
		...
		<form role="form" id="frm_modify" method="post" name="frm_board" >
            <div class="form-group">
							 <label>글번호</label>
							 <input class="form-control" name="rv_no" value='<c:out value="${review.rv_no}"/>'
							 readonly="readonly" />
					 	</div>
					 	<div class="form-group">
						 	<label>글제목</label>
						 	<input class="form-control" name="rv_title" value='<c:out value="${review.rv_title}"/>'
						 	/>
					 	</div>
					 	<div class="form-group">
							 <label>글내용</label>
							 <textarea class="form-control" rows="3" name="rv_content"
							 ><c:out value="${review.rv_content}"/></textarea>
					 	</div>
			 			<div class="form-group">
							 <label>작성자</label>
							 <input class="form-control" name="rv_writer" value='<c:out value="${review.rv_writer}"/>'
							 readonly="readonly"/>
						 </div>
            <br>
            <div class="row">
               <div class="col-12">
                  <div class="preview-list">
                     <div class="form-group">
						<br>
						<br>
						<button type="button" class="btn btn-danger btn-frmModify" id="btnModify" data-oper="modify">수정</button>
		...
		function check_upload_file(file_name, file_size) {
			...
			var clone_input_file = $(".upload_div").clone() ; //<-- 파일업로드 처리 함수위에 추가
				...
				//url 키에 명시된 주소의 컨트롤러에게 formData 객체를 POST 방식으로 전송.
				 $.ajax({
					 type: 'post',
					 url: '${contextPath}/file_upload_ajax_action',
