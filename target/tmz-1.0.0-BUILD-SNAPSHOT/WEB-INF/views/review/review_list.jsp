              ...
              <h3>후기 목록</h3> 
               ...
               <!-- 표시할 게시물 수 선택 -->
                    <label class="sr-only">review_send_values</label>
                      <select class="btn btn-outline-warning dropdown-toggle" id="select_review_row" name="review_page_row_amount_per_page"><!-- 표시 게시물 수 선택 -->
               ...
               <!-- 검색범위 설정 -->
                <select class="btn btn-outline-warning dropdown-toggle" id="select_review_scope" name="review_scope"><!-- 검색 범위 선택 -->
                ...
                <div class="input-group"><!-- 검색어 입력 -->
                         <input class="form-control" id="inputKeyword" name="review_keyword" type="text" placeholder="검색어를 입력하세요"
                         value='<c:out value="${review_paging_creatorDTO.my_review_pagingDTO.review_keyword}" />' />
                       <!--   <span class="input-group-btn">  -->  <!-- 전송버튼 -->
                         <button class="btn btn-outline-warning" type="button" id="review_search_go">
                         검&nbsp;색 &nbsp;
                         </button>
                      <!--    </span> -->
                     </div>
      ...
      <tbody>
             <c:forEach var="review" items="${review_list }">
               ...
               <c:if test="${review.rv_delflag == 0 }">
                       <tr class="move_detail"
											data-rv_no='<c:out value="${review.rv_no}"/>'>
											<td><c:out value="${review.rv_no}" /></td>
											<td style="text-align: left;">
											<c:out value="${review.rv_title}" />
											</td>
											<td><c:out value="${review.rv_writer}" /></td>
											<td><fmt:formatDate pattern="yyyy/MM/dd"
													value="${review.rv_regdate}" /><br> ${review.rv_regdate}</td>
											<td><fmt:formatDate pattern="yyyy/MM/dd"
													value="${review.rv_moddate}" /><br> ${review.rv_moddate}</td>
										</tr>
                     </c:if>
                 </c:forEach>
              </tbody>    
